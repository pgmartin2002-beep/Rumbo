...
- [ ] T001 [P] Añadir `@anthropic-ai/sdk`, `undici` y `dotenv` a `backend/package.json` y ejecutar
      `cd backend && npm install` (research.md R1–R3, R7)
- [ ] T002 [P] Crear `backend/.env.example` con `ANTHROPIC_API_KEY`, `RUMBO_AI_MODEL`,
      `RUMBO_AI_MAX_HTML_BYTES` (por defecto `2097152`) y `RUMBO_AI_MAX_CHARS` (por defecto
      `20000`) (research.md R4, R7)
- [ ] T003 Cargar `dotenv` al inicio de `backend/src/api/server.ts` (`import 'dotenv/config'` antes
...
- [ ] T004 [P] Añadir `fuente_valor: string | null` a `interface Evento` en
      `backend/src/models/index.ts` (data-model.md)
- [ ] T005 [P] Añadir `esUrlPublicaCandidata(payload: string): boolean` en
      ...
      (research.md R8)
- [ ] T006 Crear `CompositeEventExtractionAdapter implements EventExtractionAdapter` en
...
- [ ] T007 Actualizar `backend/src/services/import-service.ts`: tras una extracción exitosa
...
- [ ] T008 [P] [US1] Crear `backend/src/integrations/ssrf-guard.ts`: `esIpPrivada(ip: string)`
...
- [ ] T009 [P] [US1] Crear `backend/tests/ssrf-guard.test.ts`: casos de IP pública, privada,
      loopback y metadatos de nube para `esIpPrivada`/`resolverYValidar` (research.md R10)
- [ ] T010 [US1] Crear `backend/src/integrations/http-fetch.ts`: `obtenerHtml(url, deadline)` con el
...
- [ ] T011 [P] [US1] Crear `backend/src/services/html-to-text.ts`: `htmlATexto(html: string): string`
...
- [ ] T012 [P] [US1] Crear `backend/tests/html-to-text.test.ts`: limpieza de etiquetas/entidades y
      recorte al límite de caracteres (research.md R10)
- [ ] T013 [P] [US1] Añadir `esDatosExtraidosValidos(datos: unknown): datos is DatosEventoExtraidos`
      ...
      (research.md R6)
- [ ] T014 [US1] Crear `AnthropicEventExtractionAdapter implements EventExtractionAdapter` en
...
- [ ] T015 [US1] Completar `CompositeEventExtractionAdapter.extraerDeUrl` (placeholder de T006):
...
- [ ] T016 [US1] Actualizar `backend/src/context.ts`: leer `ANTHROPIC_API_KEY`/`RUMBO_AI_MODEL` de
...
- [ ] T017 [P] [US1] Crear `backend/tests/event-extraction.test.ts`: `AnthropicEventExtractionAdapter`
...
- [ ] T018 [P] [US2] Ampliar `backend/tests/event-extraction.test.ts` (T017):
...
- [ ] T019 [P] [US2] Ampliar `backend/tests/ssrf-guard.test.ts` (T009): `obtenerHtml` (T010)
      revalida SSRF en cada salto de redirect, incluido un redirect hacia una IP privada
      (research.md R2, R10) (depende de T010)
- [ ] T020 [US2] En `backend/src/context.ts` (T016): cuando `ANTHROPIC_API_KEY` no está definida, no
...
- [ ] T021 [P] [US2] Crear `e2e/tests/importar-url.spec.ts` cubriendo el Escenario 2 (pasos 2–4) de
...
- [ ] T022 [P] [US3] Añadir a `backend/tests/event-extraction.test.ts` (T017) un caso que llame a
...
- [ ] T023 [US3] Ejecutar sin modificar la suite E2E existente de 001/002
...
- [ ] T024 [P] Actualizar `README.md`: mencionar la feature 003, las variables de entorno nuevas del
      backend (`ANTHROPIC_API_KEY`, `RUMBO_AI_MODEL`, etc.) y el escenario E2E nuevo
- [ ] T025 [P] Ejecutar `cd backend && npm run typecheck && npm run lint` sobre todos los archivos
      nuevos/modificados
...
- [ ] T027 [P] Revisar los logs de una importación fallida y una exitosa para confirmar que ninguna
