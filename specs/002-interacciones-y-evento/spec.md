...
4. **Dado** que estoy en un hueco entre sesiones sin actividad asignada, **Cuando** creo una nota, **Entonces** la app la guarda vinculada al evento en general, señalando que no corresponde a ninguna sesión concreta. [NECESITA ACLARACIÓN: ¿debe la app ofrecer la opción de vincular manualmente esa nota a una sesión anterior o posterior?]
...
5. **Dado** que introduzco el nombre de un contacto que ya había registrado antes en el mismo evento, **Cuando** intento guardarlo de nuevo, **Entonces** la app me avisa de la posible duplicidad y me permite fusionar la información o guardarlo como un contacto distinto. [NECESITA ACLARACIÓN: ¿qué criterio usa la app para detectar que dos contactos podrían ser la misma persona — coincidencia exacta de nombre, aproximada, u otra señal?]

### Casos límite

- ¿Qué pasa si el usuario abre el modo simplificado sin conexión a internet? [NECESITA ACLARACIÓN: ¿qué parte de esta funcionalidad debe seguir disponible sin conexión y cómo se sincroniza al recuperarla?]
...
- ¿Qué ocurre si dos sesiones se solapan en la agenda (conflicto ya señalado en la Actividad 1) y el usuario abre el modo simplificado en ese hueco?
...
- ¿Cómo se gestiona un contacto registrado sin ninguna nota, solo con el nombre?
...
- **FR-011**: El sistema DEBE permitir registrar manualmente un contacto con, como mínimo, un nombre y una nota rápida.
...
- **FR-013**: El sistema DEBE vincular cada nota registrada a la sesión activa en el momento de su creación, o al evento en general si no hay ninguna sesión activa.
...
- **FR-016**: El sistema DEBE avisar al usuario cuando registra un contacto cuyo nombre coincide con uno ya existente en el mismo evento, y permitir fusionarlo o guardarlo como distinto. [NECESITA ACLARACIÓN: criterio exacto de coincidencia para considerar dos contactos como potencialmente duplicados]
- **FR-017**: El sistema DEBE permitir al usuario crear notas y registrar contactos sin conexión a internet, sincronizando los datos cuando la conexión se recupere. [NECESITA ACLARACIÓN: ¿aplica también al modo simplificado y a la generación de preguntas, o solo a la captura de notas y contactos?]
...
- **Contacto**: persona registrada por el usuario durante el evento. Atributos clave: nombre, nota rápida asociada, sesión o momento en que se conoció, posibles duplicados detectados.
...
- **SC-003**: El tiempo medio para registrar un contacto (nombre y nota) es inferior a 15 segundos.
