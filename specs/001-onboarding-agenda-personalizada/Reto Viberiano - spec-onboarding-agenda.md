...
**Creada**: 2026-07-17
**Actualizada**: 2026-07-31 — se añade la Historia de Usuario 5 (Mis eventos), la pantalla de inicio que lista, clasifica y permite navegar entre los eventos del usuario.
**Estado**: Borrador
...
Como asistente que puede tener varios eventos en distintos momentos (uno hoy, otro dentro de tres semanas, otro ya terminado), quiero ver todos mis eventos en una sola pantalla al abrir la app, para saber de un vistazo cuál está en curso, cuál tengo pendiente de preparar y cuál ya cerré, y retomar cualquiera de ellos sin tener que buscarlo.

**Por qué esta prioridad**: es la puerta de entrada de la aplicación y la que hace posible la suposición de que "un usuario puede gestionar varios eventos en paralelo" (ver Suposiciones). Sin esta pantalla no hay forma de alternar entre eventos, de retomar uno a medio configurar, ni de añadir uno nuevo sin perder los que ya existen.

**Test independiente**: se puede probar con una cuenta que tenga cero, uno o varios eventos en distintos estados (importado sin objetivos, con agenda generada, ya finalizado) y comprobando que la pantalla de inicio los clasifica y prioriza correctamente, sin depender de que ninguna otra funcionalidad esté completa.

**Escenarios de aceptación**:
1. **Dado** que tengo uno o varios eventos guardados, **Cuando** abro la app, **Entonces** veo una lista con todos mis eventos, cada uno mostrando su nombre, fechas, ubicación y su estado (en curso, próximo o cerrado).
2. **Dado** que tengo un evento que se está celebrando hoy, **Cuando** abro la app, **Entonces** ese evento aparece destacado en primer lugar como "en curso", indicando además qué actividad me corresponde ahora mismo.
3. **Dado** que tengo un evento importado al que todavía le faltan pasos (definir objetivos o generar la agenda), **Cuando** lo veo en mi lista, **Entonces** la app me muestra qué pasos del onboarding ya completé y cuáles me faltan.
4. **Dado** que estoy en la pantalla de inicio, **Cuando** selecciono cualquier evento de mi lista, **Entonces** la app me lleva al punto en el que lo dejé (a completar objetivos si me faltan, a la agenda si ya está lista, o a la actividad correspondiente si el evento está en curso).
5. **Dado** que quiero añadir un evento nuevo, **Cuando** lo indico desde la pantalla de inicio, **Entonces** la app me lleva directamente al flujo de importación (Historia de Usuario 1) sin perder de vista mis eventos existentes.
6. **Dado** que todavía no tengo ningún evento guardado, **Cuando** abro la app por primera vez, **Entonces** la app me muestra un estado de bienvenida centrado en añadir mi primer evento, en lugar de una lista vacía.
7. **Dado** que tengo eventos ya cerrados, **Cuando** los consulto en mi lista, **Entonces** puedo distinguirlos claramente de los eventos en curso o próximos y acceder a lo que capturé durante ellos (notas, contactos, etc.).
...
- **FR-026**: El sistema DEBE indicar, para cada evento cuyo onboarding no esté completo, qué pasos ya se completaron (importado, objetivos definidos, agenda generada) y cuáles quedan pendientes.
- **FR-027**: El sistema DEBE permitir iniciar la importación de un evento nuevo (Historia de Usuario 1) directamente desde la pantalla de inicio, sin perder de vista los eventos ya guardados.
- **FR-028**: El sistema DEBE llevar al usuario, al seleccionar un evento de la lista, al punto exacto en el que quedó su configuración (objetivos pendientes, agenda ya generada, o actividad en curso).
- **FR-029**: El sistema DEBE mostrar un estado de bienvenida específico, centrado en añadir el primer evento, cuando el usuario no tiene ningún evento guardado.

### Entidades clave

- **Evento**: representa el evento o challenge importado. Atributos clave: nombre, fechas, ubicación, requisitos de acceso, fuente de importación, estado derivado (en curso / próximo / cerrado, calculado a partir de las fechas), progreso de onboarding (importado / objetivos definidos / agenda generada). Relación: contiene sesiones y empresas participantes; pertenece a un usuario que puede tener varios eventos en paralelo.
...
- **SC-008**: El 95% de los usuarios identifican correctamente, sin necesidad de ayuda, cuál de sus eventos está en curso al abrir la pantalla de inicio.
- **SC-009**: El 90% de los usuarios con un evento a medio configurar retoman el paso correcto (objetivos o agenda) en menos de 5 segundos desde que abren la app.
...
- Un usuario puede gestionar varios eventos en paralelo, cada uno con su propio perfil de objetivos y su propia agenda; la Historia de Usuario 5 (Mis eventos) es la pantalla que hace visible y navegable esta suposición.
...
- Queda FUERA de alcance en esta spec: la preparación de preguntas por sesión o ponente, el copiloto de networking, la captura de contactos, las notas contextuales, el modo "durante el evento", la replanificación en tiempo real y el follow-up posterior — cada una se especificará en su propia spec. La Historia de Usuario 5 se limita a listar, clasificar y navegar entre eventos; no decide cómo se archivan ni se eliminan eventos cerrados a largo plazo.
